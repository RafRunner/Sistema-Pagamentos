		...
		usuarioService.get(cobranca.idRemetente).then(remetente => {
			usuarioService.get(cobranca.idDestinatario).then(destinatario => {
				...
				if (!autorizado) {
					...
					if (socketRemetente) {
						socketRemetente.emit('atualizacao_saldo', `Sua cobrança para ${destinatario.nome} da conta ${destinatario.numeroConta} de ${cobranca.valor} centavos foi recusada`, 0);
					}
				...
				usuarioService.transferencia(destinatario.numeroConta, remetente.numeroConta, cobranca.valor).then(resultado => {
					if (resultado.sucesso) {
						...
						if (socketRemetente) {
							socketRemetente.emit('atualizacao_saldo', `Sua cobraça de ${cobranca.valor} centavos para ${destinatario.nome} da conta ${destinatario.numeroConta} foi paga`, cobranca.valor);
						}
					}
					callback(resultado);
				});
